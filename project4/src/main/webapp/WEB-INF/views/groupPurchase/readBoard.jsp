	...
	<div id="form1">
		...
		<div id="filelist">
			...
			<c:forEach var="fname" items="${board.fnames}">
				<label onclick="downFile('${fname}')">${fname}</label>
				<br>
			</c:forEach>
		</div>
			...
			<div id="commentlisttb">
				<table>
					<c:forEach var="fcomm" items="${commlist}">
						<tr>
							<th><input type="hidden" name="fcommkey"
								value="${fcomm.fcommkey}"></th>
							<th>${fcomm.fcommkey}</th>

							<th style="text-align: left"><c:forEach varStatus="sts"
									begin="1" end="${fcomm.level}">
						&nbsp;&nbsp;
    					<c:if test="${fcomm.level>1 and sts.last }">
										<img width="20" height="10" src="${path}/a02_img/reply1.png" />
									</c:if>
								</c:forEach> ${fcomm.nickname}</th>
							<td>${fcomm.commcontent}</td>
							<th>${fcomm.commdate}</th>
							<td><button onclick="recomm(${fcomm.fcommkey})"
									class="btn btn-primary">답글</button></td>
		...
		<form id="commform" method="post" action="${path}/insertcomm.do">
			<div id="commentwrite">
				...
				<input type="hidden" name="fppkey" value="${fppkey}"> <input
					type="hidden" name="id" value="${users.id}">
				<button type="reset" class="btn btn-primary">초기화</button>
				<button type="button" id="confirmbtn" class="btn btn-primary"
					data-toggle="modal" data-target="#commmymodal">등록</button>
			</div>
		</form>
		...
		<form id="recommform" method="post" action="${path}/insertrecomm.do">
			<div id="recommentwrite">
				...
				<input type="hidden" name="fppkey" value="${fppkey}"> <input
					type="hidden" name="id" value="${users.id}"> <input
					type="hidden" name="refno">
				<button type="button" id="recancleBtn" class="btn btn-primary">취소</button>
				<button type="button" id="confirmbtn" class="btn btn-primary"
					data-toggle="modal" data-target="#recommmymodal">등록</button>
			</div>
		</form>
